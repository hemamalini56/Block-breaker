...
public class BlockBreakerPanel extends JPanel implements KeyListener{
	...
	int size=25;
	BlockBreakerPanel(){
		paddle = new Block(175,480,150,25,"slider.png");
		for(int i=0;i<8;i++){
			blocks.add(new Block((i*60+2),0,60,25,"red.png"));
		}
		for(int i=0;i<8;i++){
			blocks.add(new Block((i*60+2),25,60,25,"purple.png"));
		}
		for(int i=0;i<8;i++){
			blocks.add(new Block((i*60+2),50,60,25,"violet.png"));
		}
		for(int i=0;i<8;i++){
			blocks.add(new Block((i*60+2),75,60,25,"Dblue.png"));
		}
		for(int i=0;i<8;i++){
			blocks.add(new Block((i*60+2),100,60,25,"blue.png"));
		}
		for(int i=0;i<8;i++){
			blocks.add(new Block((i*60+2),125,60,25,"Lgreen.png"));
		}
		for(int i=0;i<8;i++){
			blocks.add(new Block((i*60+2),150,60,25,"yellow.png"));
		}
		for(int i=0;i<8;i++){
			blocks.add(new Block((i*60+2),175,60,25,"orange.png"));
		}
